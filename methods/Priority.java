...
public class Priority extends Schema {
    ...
    public Priority(ArrayList<ProcessData> processes) {
    ...
    public void schedule() {
        ...
        List<ProcessData> remaining = new ArrayList<>(processes);
        ...
        while (!remaining.isEmpty()) {
            ...
            available.sort(Comparator
                    .comparingInt((ProcessData p) -> p.priority)
                    .thenComparingInt(p -> p.arrivalTime));

            ProcessData process = available.get(0);
    ...
    public void printTable() {
        ...
        System.out.println("Priority Scheduling Table");
        System.out.printf(
                "%-10s%-10s%-10s%-10s%-10s%-12s%-10s%-12s%n",
                "Process",
                "Arrival",
                "Burst",
                "Priority",
        ...
        for (ScheduledProcess row : tableRows) {
            System.out.printf(
                    "%-10s%-10d%-10d%-10d%-10d%-12d%-10d%-12d%n",
                    row.name,
                    row.arrival,
                    row.burst,
                    row.priority,
